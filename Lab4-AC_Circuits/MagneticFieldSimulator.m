...
proximateSolenoid = true; %enable or disable the solenoid
...
for i = 1:range/step
    for j = 1:range/step
        for k = 1:range/step
            coordinates(:, n) = [i*step-range/2 j*step-range/2 k*step-range/2];
            n = n + 1;
        end
    end
end
...
for i = 1:range/step %every x point
    for j = 1:range/step %every y point
        for k = 1:range/step %every z point
