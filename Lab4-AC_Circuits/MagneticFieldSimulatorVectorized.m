...
coordinates(3, :) = repmat(coordrange, size(coord).^2);
...
for i = coordrange %every x point
    for j = coordrange %every y point
        for k = coordrange %every z point
            for m = 1:numpts %evert point for the solenoids
                %Create r vector that is from position to dL on loop
                r = [i - loop(1, m) j - loop(1, m) k - loop(3, m)];
                r2 =[i - loop2(1, m) j - loop(2, m) k - loop2(3, m)];
                r3 =[i - loop3(1, m) j - loop(3, m) k - loop3(3, m)];
                ...
                B(2, n) = B(2, n) - (d_loop(1, m)*r(3) - r(1)*d_loop(3, m)) / norm(r)^3;
                B(3, n) = B(3, n) + (d_loop(1, m)*r(2) - r(1)*d_loop(2, m)) / norm(r)^3;
                
                %Sum of the dB for each point on the loop 2 in accordance with Biot-Savart
                B(1, n) = B(1, n) + (d_loop2(2, m)*r2(3) - r2(2)*d_loop2(3, m)) / norm(r2)^3;
                B(2, n) = B(2, n) - (d_loop2(1, m)*r2(3) - r2(1)*d_loop2(3, m)) / norm(r2)^3;
                B(3, n) = B(3, n) + (d_loop2(1, m)*r2(2) - r2(1)*d_loop2(2, m)) / norm(r2)^3;
                
                if proximateSolenoid 
                    %Sum of the dB for each point on the loop 3 in accordance with Biot-Savart
                    B(1, n) = B(1, n) + (d_loop3(2, m)*r3(3) - r3(2)*d_loop3(3, m)) / norm(r3)^3;
                    B(2, n) = B(2, n) - (d_loop3(1, m)*r3(3) - r3(1)*d_loop3(3, m)) / norm(r3)^3;
                    B(3, n) = B(3, n) + (d_loop3(1, m)*r3(2) - r3(1)*d_loop3(2, m)) / norm(r3)^3;
                end
            end
